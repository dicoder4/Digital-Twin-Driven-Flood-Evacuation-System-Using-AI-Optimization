...
# Suppress MongoDB boolean check warning
...
def _get_db():
    ...
    if not mongo_url:
        print("[MONGO DEBUG] MONGODB_URI/MONGO_URL/MONGO_URI not found in environment variables.")
        return None
...
def get_shelter_cache(hobli_key: str) -> list | None:
    ...
    if doc is not None and "candidates" in doc:
...
def get_dem_cache(dem_key: str) -> bytes | None:
    ...
    if doc is not None and "data_b64" in doc:
