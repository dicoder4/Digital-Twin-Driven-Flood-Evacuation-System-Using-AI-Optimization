...
def validate_safe_centers_against_flood(safe_centers_gdf, flood_poly, edges):
    ...
    if flooded_centers:
        print(f"[WARN] Excluded {len(flooded_centers)} centers in flood zones: {', '.join(flooded_centers)}")
    
    if safe_centers:
        return gpd.GeoDataFrame(safe_centers, crs=safe_centers_gdf.crs)
    else:
        print("[ERROR] No safe centers remain after flood validation")
...
def prepare_safe_centers(hospitals_gdf, police_gdf, edges, flood_poly):
    ...
    if not safe_centers:
        print("[WARN] No hospitals or police stations found. Creating mock evacuation centers.")
    ...
    if safe_centers:
        ...
        if safe_centers_gdf.empty:
            print("[WARN] All safe centers are flooded. Generating random mock centers outside flood zones.")
            ...
            if mock_centers:
                ...
                print(f"[OK] Placed {len(mock_centers)} mock centers after {attempts} attempts.")
            else:
                print("[ERROR] Could not place mock centers outside flood zones after max attempts.")
    else:
