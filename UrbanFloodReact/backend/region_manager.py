    ...
    try:
        ...
        for row in station_rows:
            ...
            if "red" in _text(line_name):
                continue
            station_line_map[_station_key(station_name)] = line_name
        ...
        for row in network_rows:
            ...
            station_name = _sanitize_val(row.get("station_name"))
            if station_name:
                station_line_map.setdefault(_station_key(station_name), line_name)
...
def _enrich_osm_line_labels_from_reference(osm_line_features, reference_lines):
    ...
    for feature in osm_line_features:
        ...
        # ~12km threshold around Bengaluru in degree units.
        if best_line and best_dist <= 0.12:
...
def _merge_full_line_reference_segments(osm_feature_collection, reference_feature_collection):
    """
    Keep OSM geometry as primary, and append full reference segments for lines
    active in OSM so selected-line highlight is complete in frontend.
    Handles both metro AND railway lines.
    
    For railway: Include COMPLETE geometry from reference to ensure full line display.
    """
    ...
    # Track active lines for both metro AND railway
    ...
    def _feature_key(feature):
        ...
        first = tuple(coords[0]) if coords else ()
        last = tuple(coords[-1]) if coords else ()
        return (
            _text(props.get("transport_type")),
            _text(props.get("line") or props.get("name")),
            first,
            last,
            len(coords),
        )

    # Add OSM features first
    for feature in osm_features:
    ...
    # Then add reference features that match active lines (both metro and railway)
    for feature in ref_features:
        props = (feature or {}).get("properties", {}) or {}
        transport_type = props.get("transport_type", "metro")
        line_name = _text(props.get("line") or props.get("name"))
        
        # For METRO: only include if line is active in OSM
        # For RAILWAY: include if it's an active railway line from our stations
        if transport_type == "metro":
...
def _enrich_station_lines_from_network(stations, metro_lines):
    ...
    for station in stations:
        ...
        if transport_type == "metro":
            ...
            # Approx ~12 km in degrees near Bengaluru.
            if should_override and best_line and best_dist <= 0.12:
    ...
    try:
        ...
        if metro_seed_rows:
            for item in classified_rows:
                ...
                # ~12km threshold for connected KML corridor segments.
                if best is not None and best_dist <= 0.12:
