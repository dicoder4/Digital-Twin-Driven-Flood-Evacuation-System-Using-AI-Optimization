...
def run_evacuation_and_plot(algorithm_name, G, flooded_people, flood_gdf, safe_centers_gdf, 
    ...
    # Print summary
    print(f"[INFO] Flooded people: {len(flooded_people)}")
    print(f"[OK] Evacuated: {len(evacuated_people)}")
    print(f"[WARN] Unreachable: {len(unreachable)}")
    if evac_times:
        print(f"[TIME] Estimated evacuation time: Avg = {np.mean(evac_times):.2f} min | Max = {np.max(evac_times):.2f} min")
    print(f"[TIME] Script execution time: {exec_time:.2f} sec\n")
    
    print("[LOG] Evacuation Log:")
