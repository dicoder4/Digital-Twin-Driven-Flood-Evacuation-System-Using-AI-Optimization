...
### How it Works
1. **Route Analysis**: It takes the raw ACO-optimised pedestrian evacuation paths generated by the Digital Twin.
2. **GTFS Integration**: It queries the BMTC GTFS database (via `transport_agent.py` and MCP) to find the nearest physical bus stops to the flood zones.
3. **Fleet Allocation**: It dynamically chunks the required evacuees into busloads (e.g., 50 Pax/Bus) and assigns them to active transit lines mapped to safe shelters.
