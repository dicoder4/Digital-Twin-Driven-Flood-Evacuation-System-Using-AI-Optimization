...
def get_osm_features(location, tags, label):
    ...
    try:
        ...
        try:
            print(f"[WARN] Falling back to address search for {label}...")
            gdf = features_from_address(f"{label} near {location}", tags=tags)
        except Exception as e:
            print(f"[ERROR] Could not load {label} data for {location}: {e}")
            return None
    return gdf
...
def load_road_network_with_filtering(location_name, lat, lon, network_dist, filter_minor=True):
    ...
    try:
        ...
        print(f"[WARN] graph_from_place failed: {e}")
        print("[INFO] Falling back to graph_from_point() ...")
        try:
            # Use provided coordinates as fallback
            G = ox.graph_from_point((lat, lon), dist=network_dist, network_type='drive')
        except Exception as e2:
            print(f"[ERROR] graph_from_point also failed: {e2}")
            return None
    ...
    if filter_minor:
        ...
        print(f"[FILTER] Filtered out {len(edges_to_remove)} minor road segments")
